...
export class ChoiceLocator implements OnInit {
  ...
  @Output() locationUpdateForTheLastTime = new EventEmitter<{ xCord: number, yCord: number, diameter: number, element: HTMLElement }>()
  ...
  @HostListener('cdkDragEnded') locationUpdated() {
    ...
    this.locationUpdateForTheLastTime.emit(centerLocation)
